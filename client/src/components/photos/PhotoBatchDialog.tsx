...
/**
 * Determine the pixel dimensions of an image file.
 *
 * @param file - The image `File` to measure
 * @returns An object containing `width` and `height` in pixels
 * @throws An `Error` with message "Failed to load image" if the file cannot be loaded as an image
 */
...
/**
 * Create a JPEG Blob containing the specified cropped region from an image source.
 *
 * Loads the image at `imageSrc`, extracts the rectangle described by `crop`, draws it onto a canvas,
 * and converts the canvas content to a JPEG `Blob` (quality 0.95).
 *
 * @param imageSrc - URL or object URL of the source image (CORS-safe if needed)
 * @param crop - Rectangle in pixels relative to the source image ({ x, y, width, height })
 * @returns A JPEG `Blob` representing the cropped region
 * @throws If the image fails to load, the canvas 2D context cannot be obtained, or canvas-to-Blob conversion fails
 */
...
/**
 * Render a modal UI to validate, crop, and collect a batch of image files.
 *
 * Presents a step-through dialog that:
 * - validates each input file (type, minimum file size, and minimum dimensions),
 * - lets the user crop valid images, produce cropped previews, and mark images as cropped or invalid,
 * - supports navigation between images with an unsaved-crop confirmation when needed,
 * - provides a review/done state and a close confirmation that cleans up object URLs.
 *
 * @param open - Whether the dialog is visible
 * @param files - Array of input image `File` objects to process
 * @param onComplete - Called with an array of cropped `Blob` objects when the user finishes
 * @param onCancel - Called when the user confirms closing/cancelling the dialog
 * @returns The dialog element that manages the batch image cropping workflow
 */
